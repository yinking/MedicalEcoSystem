...
//    from User u join Enterprise e on u.idEnterprise=e.idEnterprise where u.role="Doctor";
//END $$
//DELIMITER ;
    ...
    public Boolean authenticate(String username, char[] password) {
        ...
        try {
            tx = session.beginTransaction();
            sqlq = session.createSQLQuery("SELECT * FROM User WHERE User.username = (:username) AND User.password = (:password)");
            sqlq.setString("username", username);
            sqlq.setString("password", String.valueOf(password));
